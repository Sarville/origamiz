...
| 7 | Иконка приложения | `electron/favicon.ico/.png/.icns` | 3 | Прямая замена |
...
`electron/favicon.ico/.png/.icns` — иконка приложения/окна, три формата одного лого.
...
Готовый результат из 7.2 (только знак, без слова) используется отдельно для:
`electron/favicon.png` (512×512, ресайзится в `.ico`/`.icns` стандартными
конвертерами) и base64-картинки в `src/html/index.html` (это тоже полный
...
| Файл | Ключ в коде | Текущая длительность | Формат | Где звучит |
|---|---|---|---|---|
| `music/menu.wav` | `MUSIC.menu` | 1:35 | 48kHz, стерео | Заставка/главное меню |
| `music/theme-full.mp3` | `MUSIC.theme` | 41:22 | 44.1kHz, стерео | Основной геймплей (фабрика) |
| `music/puzzle-full.mp3` | `MUSIC.puzzle` | 20:07 | 44.1kHz, стерео | Режим паззлов |

Важно: в движке музыка играется с `loop: true` — Howler просто прыгает на 0-ю
секунду по окончании, **без кроссфейда**. Значит начало и конец трека должны
звучать совместимо (не обрывать фразу на полутакте), иначе на длинных сессиях
будет слышен щелчок/шов на границе повтора. Длина 20–40 минут у геймплейных
треков — не техническое требование, а просто расчёт "чтобы повтор был не
заметен на долгой сессии"; можно сделать короче, если трек зациклен
действительно бесшовно, но совсем короткий (как у типичного sfx-лупа) будет
навязчивым при часах геймплея.
...
4. Проверить в деве: особенно loop-шов у `theme-full`/`puzzle-full`/`menu`, и
