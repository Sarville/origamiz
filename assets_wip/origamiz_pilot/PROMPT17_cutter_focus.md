...
1. **Platform**: a rectangle spanning the full 384x192 canvas edge-to-edge
   ...
   EXCEPT the TOP-RIGHT corner, which is cut off by a clearly large, obvious
...
2. **Curb/frame**: the same wood-post-and-rail language as `hub_current.png`
   ...
   shape exactly, including tracking the diagonal chamfer at the top-right
   (the frame corner post at top-right is itself built at that same angle —
