...
- [x] **Chunk 5 — Puzzle mode as a standalone mini-game (separate build/ship).**
      Done 2026-09-02, via a different route than originally planned here: instead
      of trimming this repo down to a puzzle-only build, puzzle mode was extracted
      into its own independent repo, `origamiz-puzzle` (sibling directory, forked
      with full history off this repo, then had regular/sandbox mode stripped from
      it as its own mirror-image cleanup). This repo had puzzle mode removed in
      full: `game/modes/puzzle*.js`, all `puzzle_*`/`next_puzzle` HUD parts,
      `states/{puzzle_menu,login}.js`, `platform/api.js` (the puzzle DLC client),
      the puzzle-only buildings (`goal_acceptor`/`constant_producer`/`block`/
      `item_producer`) with their systems/components, matching CSS/sprites/music,
      and every touchpoint in shared files (registries, toolbar, key bindings,
      savegame, all 34 translation files). The "where do puzzle levels come from"
      question is now `origamiz-puzzle`'s to answer, not this repo's.
