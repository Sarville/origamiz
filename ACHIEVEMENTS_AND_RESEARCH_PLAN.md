...
- Новый `src/js/game/hud/parts/research.js` — по образцу
  `src/js/game/hud/parts/shop.js` (апгрейды) — ближайший существующий
  референс верстки "список + прогресс-бар + кнопка покупки".
- `src/js/game/hud/parts/game_menu.js` — новая кнопка меню, видимая после
  уровня открытия Тира 1 (см. `visible:()=>hubGoals.level>9` в моде).
